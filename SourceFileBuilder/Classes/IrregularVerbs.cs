...
namespace SourceFileBuilder.Classes
{
    class IrregularVerbs
    {
        ...
        public static List<Tuple<string, string, int>> GetIrregularVerbsFormsAndAssociatedLemma(int weight = 100)
        {
            ...
            foreach (var iv in AllIrregularVerbs)
            {
                var allDerivedForms = iv.PastParticiple
                    ...
                    .Distinct()
                    .Select(s => new Tuple<string, string, int>(s.ToLowerInvariant(), iv.BaseForm.ToLowerInvariant(), weight))
                    .ToList();
